/**
 * app.js — STEP 1: UI prototípus mock adatokkal
 * ------------------------------------------------------------------
 * A fájl 4 részre tagolódik:
 *   1) SafeStorage    — localStorage, biztonságos fallback-kel
 *   2) DataStore      — "adatréteg" mock adatokon; Step 5-ben ennek
 *                        csak a belseje cserélődik Supabase hívásokra,
 *                        a rá épülő UI-logika nem változik
 *   3) state + segédfüggvények (szűrés, rendezés, szavazatszámlálás)
 *   4) Render függvények + eseménykezelők
 * ------------------------------------------------------------------
 */

/* ================== 1) SAFE STORAGE ============================ */
/**
 * A specifikáció szerint a kiválasztott nevet localStorage-ban
 * tároljuk. Néhány elzárt előnézeti környezetben (pl. sandboxolt
 * iframe) a localStorage elérése hibát dobhat — ilyenkor egy
 * egyszerű memóriabeli tárolóra esünk vissza, hogy az app ne
 * törjön el. Éles hosztingon (Cloudflare Pages / Netlify / GitHub
 * Pages) ez mindig a valódi localStorage-ot fogja használni.
 */
const SafeStorage = (() => {
  let memoryFallback = {};
  let storageOk = false;
  try {
    const testKey = '__noszvaj_test__';
    ...
    set(key, value) {
      if (storageOk) {
        try {
          ...
          /* esik át a memóriás fallback-re */
        }
      }
...
/* ================== 2) DATA STORE (mock) ======================== */
/**
 * Ez a réteg fedi le mindazt, amit Step 5-ben Supabase hívások
 * fognak adni (select / insert / update / delete). Most szinkron
 * memóriaműveletek async wrapperben — így a hívó kód (toggleVote,
 * init) már most is `await`-tel dolgozik, és nem kell módosítani,
 * amikor a belseje ténylegesen Supabase-t fog hívni.
 */
const DataStore = (() => {
  let votes = MOCK_VOTES.map((v) => ({ ...v }));
  let voteSeq = votes.length;
  ...
  return {
    ...
    async upsertVote(programId, userName, voteType) {
      ...
      const idx = votes.findIndex(
        (v) => v.program_id === programId && v.user_name === userName
      );
      if (idx >= 0) {
        votes[idx] = { ...votes[idx], vote_type: voteType, updated_at: now };
      } else {
        voteSeq += 1;
        votes.push({
          id: 'v' + voteSeq,
...
const state = {
  ...
  sortBy: 'liked',
  sortReversed: false,
  currentView: 'programok', // 'programok' | 'kedvencek' | 'tervezett' | 'menu'
  activeDetailId: null,
};
...
function formatKm(km) {
  // magyar tizedesvessző (1.2 -> "1,2"), felesleges tizedesjegy nélkül
  return String(km).replace('.', ',');
}

function getVotesForProgram(programId) {
  return state.votes.filter((v) => v.program_id === programId);
}

function getUserVote(programId, userName) {
  const v = state.votes.find(
    (x) => x.program_id === programId && x.user_name === userName
  );
  return v ? v.vote_type : null;
}

function getLikeCount(programId) {
  return state.votes.filter((v) => v.program_id === programId && v.vote_type === 'like').length;
}

function getDislikeCount(programId) {
  return state.votes.filter((v) => v.program_id === programId && v.vote_type === 'dislike').length;
}
...
function getSortedPrograms() {
  ...
  list.sort((a, b) => {
    ...
    if (state.sortBy === 'liked') {
      const likeDiff = getLikeCount(b.id) - getLikeCount(a.id); // több like elöl
      cmp = likeDiff !== 0 ? likeDiff : getDislikeCount(a.id) - getDislikeCount(b.id); // kevesebb dislike elöl
    } else if (state.sortBy === 'closest') {
      cmp = a.distance_km - b.distance_km;
    } else if (state.sortBy === 'shortest') {
      cmp = a.duration_hours_min - b.duration_hours_min;
    } else if (state.sortBy === 'price') {
      cmp = a.price_sort_value - b.price_sort_value;
    }
    return cmp * dir;
  });
...
async function toggleVote(programId, voteType) {
  ...
  if (existing === voteType) {
    await DataStore.deleteVote(programId, state.currentUser); // visszavonás
  } else {
    await DataStore.upsertVote(programId, state.currentUser, voteType); // új szavazat / váltás
  }

  state.votes = await DataStore.getVotes();
  renderProgramList();
  if (state.activeDetailId === programId) {
...
// globálisan elérhetővé tesszük, mert inline onerror="" attribútumból hívjuk
window.handleImgError = handleImgError;

function renderLogos() {
  // Egyszerű, saját Noszvaj-témájú jelzés: domb + fa + nap, zöld tónusban.
  const logoSvg = `
...
function renderNameSelect() {
  ...
  list.innerHTML = USERS.map(
    ...
    <button class="name-btn" data-name="${escapeHtml(name)}" type="button">
      ...
      <span class="name-btn__dot" aria-hidden="true"></span>
    </button>`
  ).join('');
...
function renderCategoryBar() {
  ...
  el.innerHTML = CATEGORIES.map(
    (cat) => `
    <button
      class="category-pill ${state.activeCategory === cat.id ? 'is-active' : ''}"
      data-category="${cat.id}"
      type="button"
      aria-pressed="${state.activeCategory === cat.id}"
    >${icon(cat.icon, { size: 16 })}<span>${cat.label}</span></button>`
  ).join('');
  ...
  <article class="program-card" data-id="${p.id}">
    <div class="program-card__main" data-role="open-detail">
      <div class="program-card__media">
        ...
        />
        <div class="program-card__media-fallback" data-category="${p.category}">
          ${icon('image', { size: 42 })}
        </div>
      </div>
      <div class="program-card__content">
        <div class="program-card__info">
          <span class="category-tag" data-category="${p.category}">
            ${icon(cat.icon, { size: 13 })}<span>${cat.label}</span>
          </span>
        ...
        <div class="program-card__votes">
          <button
            class="vote-btn vote-btn--like ${myVote === 'like' ? 'is-active' : ''}"
            data-action="like" data-program="${p.id}" type="button"
            aria-pressed="${myVote === 'like'}" aria-label="Like erre: ${escapeHtml(p.title)}"
          >${icon('thumbUp', { size: 17 })}<span>${likeCount}</span></button>
          <button
            class="vote-btn vote-btn--dislike ${myVote === 'dislike' ? 'is-active' : ''}"
            data-action="dislike" data-program="${p.id}" type="button"
            aria-pressed="${myVote === 'dislike'}" aria-label="Dislike erre: ${escapeHtml(p.title)}"
          >${icon('thumbDown', { size: 17 })}<span>${dislikeCount}</span></button>
        </div>
      </div>
    </div>
  ...
  if (list.length === 0) {
    el.innerHTML = `
      <div class="empty-state">
        ${icon('grid', { size: 34 })}
        <p>Ebben a kategóriában egyelőre nincs program.</p>
      </div>`;
    return;
  }
...
function voteButtonsHtml(p, likeCount, dislikeCount, myVote) {
  return `
    <button class="vote-btn vote-btn--like ${myVote === 'like' ? 'is-active' : ''}" data-action="like" data-program="${p.id}" type="button" aria-pressed="${myVote === 'like'}">
      ${icon('thumbUp', { size: 17 })}<span>${likeCount}</span>
    </button>
    <button class="vote-btn vote-btn--dislike ${myVote === 'dislike' ? 'is-active' : ''}" data-action="dislike" data-program="${p.id}" type="button" aria-pressed="${myVote === 'dislike'}">
      ${icon('thumbDown', { size: 17 })}<span>${dislikeCount}</span>
    </button>`;
}
...
function voterListHtml(votes, type) {
  ...
  return `
    <div class="voters-group voters-group--${type}">
      ...
      ${
        items.length
          ? `<ul>${items
              .map(
                (v) =>
                  `<li class="${v.user_name === state.currentUser ? 'is-me' : ''}">${escapeHtml(v.user_name)}${v.user_name === state.currentUser ? ' (te)' : ''}</li>`
              )
              .join('')}</ul>`
          : `<p class="voters-empty">${emptyLabel}</p>`
      }
    </div>`;
}
...
function renderDetailBody(programId) {
  ...
  document.getElementById('sheet-body').innerHTML = `
    <div class="sheet-media">
      <img src="${p.image_url}" alt="${escapeHtml(p.title)}" onerror="handleImgError(this)" />
      <div class="program-card__media-fallback" data-category="${p.category}">${icon('image', { size: 54 })}</div>
    </div>
    <div class="sheet-content">
      <span class="category-tag" data-category="${p.category}">${icon(cat.icon, { size: 13 })}<span>${cat.label}</span></span>
      ...
      <div class="sheet-meta-grid">
        ...
        <div class="sheet-meta-item">${icon('clock', { size: 18 })}<div><strong>${p.duration}</strong><span>Időtartam</span></div></div>
      ...
      <div class="voters-section">
        <h3>${icon('users', { size: 17 })}Ki hogyan szavazott?</h3>
        ${voterListHtml(votes, 'like')}
        ${voterListHtml(votes, 'dislike')}
      </div>
    </div>`;
}
...
function bindProgramListEvents() {
  document.getElementById('program-list').addEventListener('click', (e) => {
    ...
    if (voteBtn) {
      toggleVote(Number(voteBtn.dataset.program), voteBtn.dataset.action);
      return;
    }
    if (e.target.closest('.card-link')) return; // hagyjuk a natív link-megnyitást
    const openTarget = e.target.closest('[data-role="open-detail"]');
    if (openTarget) {
      const card = openTarget.closest('.program-card');
...
function bindDetailSheetEvents() {
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && !document.getElementById('detail-sheet').classList.contains('hidden')) {
      closeDetail();
    }
  });
  // szavazás közvetlenül a részletnézetből is
...
async function init() {
  ...
  // ha nincs mentett név, a name-select-overlay marad látható (alap állapot)
}
