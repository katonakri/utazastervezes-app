...
(() => {
  ...
  function getDir() { return currentDir; }
  ...
  function updateDirectionButton() {
    ...
    button.setAttribute('aria-label', desc ? 'Csökkenő sorrend' : 'Növekvő sorrend');
  }
  ...
  function bindFilterSort() {
    document.querySelectorAll('[data-bring-filter]').forEach(button => {
      ...
      button.addEventListener('click', () => window.setTimeout(sortCards, 50));
    });
  }
  ...
  function ensureControls() {
    ...
    if (!controls) {
      ...
      controls.innerHTML = `
        ...
        <button id="bring-sort-direction" type="button" aria-label="Csökkenő sorrend">↓</button>`;
      ...
      controls.querySelector('#bring-sort-direction').addEventListener('click', () => {
        currentDir = getDir() === 'desc' ? 'asc' : 'desc';
  ...
  function installShowHook() {
    ...
    const wrappedShow = function () {
      ...
      // The list is populated asynchronously from Supabase, so perform a
      // small number of one-shot sorts after rendering rather than observing DOM changes.
      window.setTimeout(ensureControls, 0);
      window.setTimeout(sortCards, 250);
      window.setTimeout(sortCards, 800);
      return result;
    };
