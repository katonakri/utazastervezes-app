...
(() => {
  const SORT_KEY = 'bring_list_sort_v3';
  const DIR_KEY = 'bring_list_sort_dir_v3';
  ...
  function sortCards() {
    ...
    const cards = [...list.querySelectorAll('.bring-card[data-bring-id]')]; if (cards.length < 2) return;
    const sort = getSort(), dir = getDir() === 'desc' ? -1 : 1;
    cards.sort((a,b) => {
      const an = a.querySelector('h3')?.textContent || '', bn = b.querySelector('h3')?.textContent || '';
      let result = sort === 'name' ? an.localeCompare(bn, 'hu') : (assigneeRank(a) - assigneeRank(b) || an.localeCompare(bn, 'hu'));
      return result * dir;
    });
    const fragment = document.createDocumentFragment(); cards.forEach(card => fragment.appendChild(card)); list.appendChild(fragment);
  }
  function updateDirectionButton() {
    const button = document.getElementById('bring-sort-direction'); if (!button) return;
    const desc = getDir() === 'desc'; button.textContent = desc ? '↓' : '↑'; button.setAttribute('aria-label', desc ? 'Csökkenő sorrend' : 'Növekvő sorrend');
  }
  function bindFilterSort() {
    document.querySelectorAll('[data-bring-filter]').forEach(button => {
      if (button.dataset.sortBound === '1') return; button.dataset.sortBound = '1'; button.addEventListener('click', () => window.setTimeout(sortCards, 0));
    });
  }
  function ensureControls() {
    const view = document.querySelector('.bring-view'), filters = view?.querySelector('.bring-filters'); if (!view || !filters) return;
    let controls = document.getElementById('bring-sort-controls');
    if (!controls) {
      controls = document.createElement('div'); controls.id = 'bring-sort-controls'; controls.className = 'bring-sort-controls';
      ...
      const select = controls.querySelector('#bring-sort-select'); select.addEventListener('change', () => { save(SORT_KEY, select.value); sortCards(); });
    ...
    controls.querySelector('#bring-sort-select').value = getSort(); updateDirectionButton(); bindFilterSort(); sortCards();
  }
  function installShowHook() {
    ...
    const originalShow = window.BringList.show; const wrappedShow = function () { const result = originalShow.apply(this, arguments); window.setTimeout(ensureControls, 0); return result; }; wrappedShow.__sortWrapped = true; window.BringList.show = wrappedShow;
  }
